...
#removes account
def delete(location):
     con = ac.messagebox.askquestion("Delete Account",f"Do you want to delete {ac_index[location].name_account()}")
     if con =='yes':
...
def login_check(frame,name,pin):
     ...
     if len(name)<=0 or len(pin)<8 or any(inter.isalpha() for inter in pin):
          ...
          for x in range(len(ac_index)):
               if (ac_index[x].get("name") == name or ac_index[x].get("email")== name) and ac_index[x].get("pin") == pin:
...
def main():
     ...
     if not login_state :
          if ac_created:
               ...
               account_create_text.grid(row=0,column=0,columnspan=5)
